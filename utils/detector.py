...
import argparse
import cv2
import utils.imgcomparison as imgcomparison
import utils.timeline as timeline
import utils.mediaoutput as mediaoutput
import utils.ui as ui

from utils.slides import Slide
...
class Detector(Analyzer):
    ...
    def __init__(self, device, outpath=None, fileformat=".png", threshold=0.99):
    ...
    def detect_slides(self, callback=None):
        ...
        for i, frame in self.check_transition():
            progress.update(i)
            callback(i / self.sequence.len*100) if callback is not None else None
